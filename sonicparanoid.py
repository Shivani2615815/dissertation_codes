...
def get_params(softVersion):
    ...
    parser = argparse.ArgumentParser(description='SonicParanoid %s'%(softVer$
    ...
    parser.add_argument('-m', '--mode', required=False, help='SonicParanoid $
    parser.add_argument('-p', '--project-id', type=str, required=False, help$
    parser.add_argument('-sh', '--shared-directory', type=str, required=Fals$
    parser.add_argument('-db', '--mmseqs-dbs', type=str, required=False, hel$
    parser.add_argument('-t', '--threads', type=int, required=False, help='N$
 #parser.add_argument('-c', '--cutoff', type=int, required=False, help='C$
    parser.add_argument('-se', '--sensitivity', type=float, required=False, $
    parser.add_argument('-noidx', '--no-indexing', required=False, help='Avo$
    parser.add_argument('-ot', '--overwrite-tables', required=False, help='T$
    parser.add_argument('-ow', '--overwrite', required=False, help='Overwrit$
    #parser.add_argument('-u', '--update', type=str, required=False, help='U$
    parser.add_argument('-ml', '--max-len-diff', type=float, required=False,$
    parser.add_argument('-mgs', '--max-gene-per-sp', type=int, required=Fals$
    parser.add_argument('-sm', '--skip-multi-species', required=False, help=$
    parser.add_argument('-op', '--output-pairs', required=False, help='Outpu$
    parser.add_argument('-qfo11', '--qfo-2011', required=False, help='Output$
    parser.add_argument('-ka', '--keep-raw-alignments', required=False, help$
    parser.add_argument('-rs', '--remove-old-species', required=False, help=$
    parser.add_argument('-un', '--update-input-names', required=False, help=$
    parser.add_argument('-d', '--debug', required=False, help='Output debug $
...
def check_gcc():
    ...
    if not shOut is None:
        ...
        print('ERROR: you must install gcc version 5.1 or above before conti$
        return (None, False)
...
def check_quick_multiparanoid_installation(gccVer=None, gccOk=False):
    ...
    if not (qa1ok and qa2ok):
        if not gccOk:
            sys.stderr.write('\nERROR: no gcc compiler was found in your sys$
            sys.stderr.write('Please, go to https://gcc.gnu.org\n')
            sys.stderr.write('And follow the instructions to install the lat$
            sys.exit(-5)
...
def check_mmseqs_installation(root, debug=False):
  ...
  # final mmseqs path
        ...
        process = subprocess.Popen(mmseqsCmd, shell=True, stdout=subprocess.$
        stdout_val, stderr_val = process.communicate() #get stdout and stderr
        process.wait()
        # check if an error happened
        mmseqsVersion = "AVX2"
        if len(stderr_val.decode()) > 0:
            print("INFO: Your system does not support the AVX2 instruction s$
...
def get_mmseqs_supported_version(readmePath):
    ...
    if not os.path.isfile(readmePath):
        sys.stderr.write('\nERROR: the file with MMseqs2 version information$
        sys.stderr.write('\nProvided path:\n{:s}\n'.format(readmePath))
        sys.exit(-5)
...
def get_input_paths(inDir, debug=False):
    ...
    if len(fname2path) < 2:
        sys.stderr.write('ERROR: the directory with the input files only con$
        sys.exit(-5)
...
def set_project_id(rId: str="", runsDir: str=os.getcwd(), debug=False):
    ...
    if len(rId) == 0:
        ...
        # minutes: tm_min
        # seconds: tm_sec
        startTime = "{:d}{:d}{:s}{:d}{:d}{:d}".format(ltime.tm_mday, ltime.t$
        ...
        for f in os.listdir(runsDir):
            ...
            if os.path.isdir(tmpPath):
                if f == rId:
                    sys.stderr.write('\nERROR: the run ID {:s} was used in a$
                    sys.stderr.write('Remove the previous run or choose a di$
                    sys.exit(-5)
    return rId
########### MAIN ############
def main():
    ...
    #### FORCE IT TO OK
    # check_quick_multiparanoid_installation('FORCED TO TRUE: Sytem specific$
    ####
 ...
 # Pair-wise tables directory
    ...
    if update_run:
        ...
        if not os.path.isfile(snapshotFile):
            sys.stderr.write("\nWARNING: not snapshot file found. The run wi$
            update_run = False
    else:
...
runMode = args.mode
 ...
 os.unlink(fpath)
    ...
    if args.sensitivity:
        if 1. <= args.sensitivity <= 7.5:
            ...
            print('WARNING: the run mode \'%s\' will be overwritten by the c$
        else:
            sys.stderr.write('\nERROR: the sensitivity parameter must have a$
    ...
    if args.max_len_diff != 0.5:
        if not (0. <= args.max_len_diff <= 1.0):
            sys.stderr.write('\nERROR: the length difference ratio must have$
 ...
 # name for multispecies groups
    ...
    print('\nSonicParanoid {:s} will be executed with the following paramete$
...
infoDict["Max length difference for in-paralogs:"] = str(args.max_len_di$
    infoDict["Overwrite pair-wise ortholog tables:"] = str(args.overwrite_ta$
    infoDict["Complete overwrite:"] = str(args.overwrite)
    infoDict["Skip creation of ortholog groups:"] = str(args.skip_multi_spec$
    if debug:
        infoDict["Directory with SonicParanoid runs:"] = str(runsDir)
        infoDict["Maximum gene per species in groups:"] = str(args.max_gene_$
    ...
    if not update_run:
        if debug:
            print("First run!")
 # Compute digests
        digestDict, repeatedFiles = idmapper.compute_hash_parallel(inPaths, $
        spFile, mappedInputDir, mappedInPaths = idmapper.map_hdrs_parallel(i$
        # create the snapshot file
        copy(spFile, snapshotFile)
        # predict pairwise orthology
        spFile, pairsFile, requiredPairsDict = orthodetect.run_sonicparanoid$
        # remap the pairwise relations
        remap.remap_pairwise_relations_parallel(pairsFile, runDir=runDir, or$

        # run multi-species clustering
        if not skipMulti:
            # copy sqltables
            sqlPaths = orthogroups.fetch_sql_files(rootDir=pairwiseDbDir, ou$
 ...
 orthoGrps, grpsStatPaths = orthogroups.run_quickparanoid(sqlTblD$
            # load the mapping information
            id2SpDict, new2oldHdrDict = idmapper.load_mapping_dictionaries(r$
            # remap the genes in orthogroups
            idmapper.remap_orthogroups(inTbl=orthoGrps, id2SpDict=id2SpDict,$
            # remap the flat multi-species table
            flatGrps = os.path.join(multiOutDir, "flat.{:s}".format(os.path.$
            idmapper.remap_flat_orthogroups(inTbl=flatGrps, id2SpDict=id2SpD$
            # remap file with not grouped genes
            notGroupedPath = os.path.join(multiOutDir, "not_assigned_genes.{$
            idmapper.remap_not_grouped_orthologs(inPath=notGroupedPath, id2S$
            # remap stats
            idmapper.remap_group_stats(statPaths=grpsStatPaths, id2SpDict=id$
            sys.stdout.write('Ortholog groups creation elapsed time (seconds$
    # Run update
    else:
        if debug:
            print("Update run!")
        # update the run info
 spFile, mappedInputDir, mappedInPaths = idmapper.update_run_info(inP$
        ...
        if not skipMulti:
            ...
            quickparaRoot = orthogroups.get_quick_multiparanoid_src_dir()
             #create the multi-species clusters
            orthoGrps, grpsStatPaths = orthogroups.run_quickparanoid(sqlTblDir=multiOutDir, outDir=multiOutDir, sharedDir=runDir, srcDir=quickparaRoot, outN$
            # load the mapping information
...
with open(pairsFile, "r") as ifd:
    ...
    if output_relations:
        ...
        # Extract pairs
        orthodetect.extract_ortholog_pairs(rootDir=os.path.join(runDir, "pai$
